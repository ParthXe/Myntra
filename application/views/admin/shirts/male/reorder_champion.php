...
$(function() {
	...
	$('.testimonials_sortable').sortable({
        ...
        update: function(event, ui) {
            ...
            // return false;
            $.ajax({
                url: base_url+"shirts/reorder_images",
                type: 'POST',
            ...
            <div class="col-xs-12">
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Change details for Shirts <?php echo $shirt_male['champion_products_title'] ;?></h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
					<?php if (isset($error)) : ?>
					...
					<?php echo form_open('', 'class="form-user-edit" role="form" autocomplete="off" enctype="multipart/form-data"') ; ?>
						<input type = "hidden" id="testimonial_id" name = "testimonial_id" value ="<?php echo $shirt_male['id'];?>">
						<div class="form-group">
							<label for="userEditMobile">Champions Images</label>
							<ul class="testimonials_sortable">
							<?php if($images = $shirt_male['champion_products_images']){
								$image = explode(",", $images);
								for($ii=0;$ii<count($image);$ii++) {
								echo '<li id='.$ii.'><span></span><img src="'.base_url().'myntra/section_products/pro_shirts/champion_product_images/'.$image[$ii].'" width="150px"></li>';
								}
						...
						<br>
                        <a class="btn btn-primary" href="<?php echo base_url("admin/shirts/male_edit/".$shirt_male['id']); ?>">Back</a>
                </div>
                <!-- /.box-body -->
              </div>
              <!-- /.box -->
