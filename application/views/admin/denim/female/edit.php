 ...
 function imageRemove(img,id,action)
{
    $.ajax({
         type: "POST",
         url: base_url+"denim/remove_femaleimage",
            ...
            <div class="col-xs-12">
              <div class="box">
                <div class="box-header">
                  <h3 class="box-title">Change details for Denim male <?php echo $denim_female['champion_products_title'] ;?></h3>
                </div>
                <!-- /.box-header -->
                <div class="box-body">
					<?php if (isset($error)) : ?>
...
<!-- 						<div class="form-group">
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" id="championsProductsTitle" name="champions_products_title1" placeholder="Champions Products Title" autocomplete="off" value="<?php echo isset($denim_female['champion_products_title']) ? $denim_female['champion_products_title'] : "NA"; ?>" required>
						</div>
						<div class="form-group">
							<label for="userEditMail">Champions Products Description</label>
							<textarea class="form-control" id="destinationDesc" name="champions_products_desc" placeholder="Champions Products Description" required><?php echo isset($denim_female['champion_products_desc']) ? $denim_female['champion_products_desc'] : "NA"; ?></textarea>
						</div>
						<div class="form-group">
							...
							<?php if($champion_images = $denim_female['champion_products_images']){
								$champion_image = explode(",", $champion_images);
								foreach ($champion_image as $champion_img) {
								echo '<img src="'.base_url().'upload/denim/female/champion_products/'.$champion_img.'" width="150px"><a href="#" onclick="imageRemove('."'".$champion_img."'".','."'".$denim_female['id']."'".','."'champion-image'".')"><i class="fa fa-times" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;';
								}
							}
							else
							{
						...
						<div class="form-group">
							...
							<input type="text" class="form-control" id="HowFar" name="trends_launch_date" placeholder="Trends Launch Date" value="<?php echo isset($denim_female['trends_launch_date']) ? $denim_female['trends_launch_date'] : "NA"; ?>" required>
						</div>
						<div class="form-group">
							<label for="userEditMobile">Trends Title</label>
							<input type="text" class="form-control" id="BestTimeVisit" name="trends_title" placeholder="Trends Title" value="<?php echo isset($denim_female['trends_title']) ? $denim_female['trends_title'] : "NA"; ?>" required>
						</div>
						<div class="form-group">
							<label for="userEditMobile">Trends Images</label>
							<input type="file" class="form-control" name="trendsImages[]" value="" multiple>
							<?php if($trends_images = $denim_female['trends_images']){
								$trends_image = explode(",", $trends_images);
								foreach ($trends_image as $trends_img) {
								echo '<img src="'.base_url().'upload/denim/female/trend_images/'.$trends_img.'" width="150px"><a href="#" onclick="imageRemove('."'".$trends_img."'".','."'".$denim_female['id']."'".','."'trends_img'".')"><i class="fa fa-times" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;';
								}
							}
							else
							{
						...
						<div class="form-group">
							...
							<?php if($vintage_images = $denim_female['vintage_images']){
								$vintage_image = explode(",", $vintage_images);
								foreach ($vintage_image as $vintage_img) {
								echo '<img src="'.base_url().'upload/denim/female/vintage_images/'.$vintage_img.'" width="150px"><a href="#" onclick="imageRemove('."'".$vintage_img."'".','."'".$denim_female['id']."'".','."'vintage_img'".')"><i class="fa fa-times" aria-hidden="true"></i></a>&nbsp;&nbsp;&nbsp;';
								}
							}
							else
							{
						...
						<div class="form-group">
							...
							<textarea class="form-control" id="vintageDescription" name="vintage_description" placeholder="Vintage Description" required><?php echo isset($denim_female['vintage_desc']) ? $denim_female['vintage_desc'] : "NA"; ?></textarea>
						</div>				
		                ...
		                <div class="form-group">
		                    ...
		                    <div class="checkbox">
		                      <label class="no-padding">
		                        <input type="checkbox" class="minimal themed" name="active" <?php echo (isset($denim_female['active']) && $denim_female['active'] == 1)  ? "checked='checked'" : "" ; ?>> Active
		                      </label>
		                    </div>
		                </div>
